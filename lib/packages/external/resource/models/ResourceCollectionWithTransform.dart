...
class ResourceCollectionWithTransform<T extends IResourceData, U extends Ext<T>> {
  ...
  Future<T?> save(T record, {bool refresh = false, local = false}) async {
    var _result = local ? record : await _adapter.save(record);

    if (_result == null) return _result;
    _dataPages[0].insert(0, _transform(_result));

    if (!_options.reactiveRecords) {
      _addRecord(_transform(_result), true);
